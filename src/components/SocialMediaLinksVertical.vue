<template>
  <div class="social-links-top flex">
    <div class="social-links--top-left social-links 
    flex flex-col items-center gap-1
    [font-size:1.3em] pl-3 pt-1 shrink-0 hover:bg-[rgba(235,182,180,0.21)] border-1 border-white/[0.25] hover:border-[rgba(247,247,247,0.26)] rounded-[23px]">
      
      <span class="bg-white/20 hover:bg-white/45 pl-3 sm:pl-4 sm:pr-2 -ml-2 mr-1 rounded-3xl">
        
        <a href="https://www.facebook.com/mczvc12"
        ><i class="fa fa-facebook mr-3"/><span class="hidden sm:inline-block font-merriweather text-white/50">facebook</span></a>
      </span>
      
      <span class="bg-white/20 hover:bg-white/45 pl-3 sm:pl-4 sm:pr-1 -ml-2 mr-1 rounded-3xl">
        <a href="https://www.instagram.com/mczvc12/"
        ><i class="fa fa-instagram mr-3"/><span class="hidden sm:inline-block font-merriweather text-white/50">instagram</span></a>
      </span>
      
      <span class="bg-white/20 hover:bg-white/45 pl-3 sm:pl-4 sm:pr-1 -ml-2 mr-1 rounded-3xl">
        <a href="#"><i class="fa fa-youtube mr-3"/><span class="hidden sm:inline-block font-merriweather text-white/50">youtube</span></a>
      </span>

      <span class="bg-white/20 hover:bg-white/45 pl-3 sm:pl-4 sm:pr-1 -ml-2 mr-1 rounded-3xl">
        <a href="https://www.twitter.com/mczvc12"
        ><i class="fa fa-twitter mr-3"/><span class="hidden sm:inline-block font-merriweather text-white/50">twitter</span></a>
      </span>

      <a class="social-links__github sm:py-2 md:p-2 text-black font-merriweather font-bold text-xl flex items-center bg-sky-200/[0.35] hover:bg-[rgba(90,167,204,0.71)] rounded-full ml-[-8px] [transition:background-color_ease-in-out_240ms] flex " href="https://github.com/mczvc-biomew"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="32" height="32" viewBox="0 0 64 64" class="github-icon">
      <path fill="#8b8e98" d="M31 3A28 28 0 1 0 31 59A28 28 0 1 0 31 3Z"/><path fill="#7e8189" d="M31,54A23,23,0,1,1,54,31,23,23,0,0,1,31,54Z"/><path fill="#faefde" d="M36.08,43.35A11,11,0,0,1,39,50v5.25c0,.55.23,2.13.9,2.31L31,59.18l-9-1.61c.72-.16,1-1.77,1-2.32s0-.91,0-3.25c-7.79,1.69-9.4-4.3-9.4-4.3-1.28-3.23-3.11-4.1-3.11-4.1-2.54-1.73.19-1.7.19-1.7A5.89,5.89,0,0,1,15,44.79c2.5,4.27,6.44,3.92,8,3.21.26-1.81,2.1-3.93,2.9-4.63-6.22-.71-12.76-3.11-12.76-13.84A10.83,10.83,0,0,1,16,22a10.09,10.09,0,0,1,.28-7.41s2.35-.75,7.7,2.87a26.61,26.61,0,0,1,14,0c5.34-3.62,7.69-2.87,7.69-2.87A10.09,10.09,0,0,1,46,22a10.83,10.83,0,0,1,2.87,7.51C48.86,40.29,42.32,42.66,36.08,43.35Z"/><path fill="#8d6c9f" d="M31,2a29,29,0,0,0-9.54,56.38,1,1,0,0,0,.55.19A29,29,0,1,0,31,2ZM23.7,57a6.22,6.22,0,0,0,.3-1.74v-.54c0-.45,0-1.06,0-2.72a1,1,0,0,0-1.21-1c-6.72,1.46-8.17-3.38-8.23-3.58l0-.11a9.44,9.44,0,0,0-3-4.24,5.37,5.37,0,0,1,2.56,2.2c2.64,4.53,7,4.65,9.3,3.62a1,1,0,0,0,.58-.77,8.62,8.62,0,0,1,2.56-4A1,1,0,0,0,26,42.38c-5.87-.67-11.87-2.72-11.87-12.84a9.82,9.82,0,0,1,2.62-6.84A1,1,0,0,0,17,21.64,8.84,8.84,0,0,1,17,15.55c.85,0,2.92.39,6.42,2.76a1,1,0,0,0,.83.14,25.56,25.56,0,0,1,13.48,0,1,1,0,0,0,.83-.14c3.51-2.38,5.59-2.74,6.41-2.77a8.87,8.87,0,0,1,.07,6.1,1,1,0,0,0,.19,1.05,9.82,9.82,0,0,1,2.61,6.84c0,10.13-6,12.17-11.89,12.82a1,1,0,0,0-.54,1.75A10.2,10.2,0,0,1,38,50v5.25A6.28,6.28,0,0,0,38.29,57,26.86,26.86,0,0,1,23.7,57Zm16.5-.6a4.13,4.13,0,0,1-.2-1.13V50a11.66,11.66,0,0,0-2-5.91c5.55-.94,11.89-3.82,11.89-14.56a11.76,11.76,0,0,0-2.74-7.73,11.44,11.44,0,0,0-.48-7.56,1,1,0,0,0-.62-.58c-.28-.09-2.85-.76-8.18,2.74a27.61,27.61,0,0,0-13.66,0c-5.33-3.5-7.9-2.83-8.18-2.74a1,1,0,0,0-.62.58,11.42,11.42,0,0,0-.48,7.56,11.75,11.75,0,0,0-2.74,7.73c0,10.65,6.23,13.58,11.73,14.56a9.51,9.51,0,0,0-1.74,3.16c-1.42.4-4.34.41-6.29-2.94a6.9,6.9,0,0,0-5.15-3.4c-.54,0-1.9.09-2.22,1.13s.91,2,1.46,2.4l.13.07c.06,0,1.52.8,2.6,3.53.23.71,2.22,6.1,9.35,5.15,0,.79,0,1.19,0,1.5v.56a4,4,0,0,1-.21,1.13,27,27,0,1,1,18.41,0Z"/><path fill="#8d6c9f" d="M31 37a1 1 0 0 0-1 1v2a1 1 0 0 0 2 0V38A1 1 0 0 0 31 37zM35.6 39.53A1 1 0 0 0 37.53 39L37 37.08a1 1 0 0 0-1.93.52zM21.75 34.52a1 1 0 0 0-1.37.37l-1 1.73a1 1 0 0 0 1.73 1l1-1.73A1 1 0 0 0 21.75 34.52zM40.88 37.62a1 1 0 1 0 1.73-1l-1-1.73a1 1 0 0 0-1.73 1zM26.21 36.37a1 1 0 0 0-1.22.71L24.47 39a1 1 0 1 0 1.93.52l.52-1.93A1 1 0 0 0 26.21 36.37zM41.35 9.46a22.71 22.71 0 0 0-2.3-1 1 1 0 0 0-.7 1.87 21.1 21.1 0 0 1 2.1.92 1 1 0 0 0 .45.11 1 1 0 0 0 .45-1.89zM34.65 7.29a23.08 23.08 0 0 0-15 2.72 1 1 0 0 0 .5 1.87 1 1 0 0 0 .49-.13A21.1 21.1 0 0 1 34.34 9.27a1 1 0 1 0 .32-2z"/>
      </svg>
      <span class="text-[1rem] sm:text-[1.4rem] block sm:hidden">GH</span>
      <span class="text-[1rem] sm:text-[1.4rem] hidden sm:block">Github</span>
    </a>
    </div>
    
    
  
  </div>
</template>

<style lang="less" scoped>
.social-links {
  @media (min-width: 640px) {
    font-size: 2.2rem;
  }
  @media (min-width: 768px) {
    font-size: 3rem;
  }

  transition: background-color cubic-bezier(0.075, 0.82, 0.165, 1) 1s;
  
  &:hover > span:hover {
    transition: transform cubic-bezier(0.075, 0.82, 0.165, 1) 1s;
    transform: rotateZ(10deg);
  }
  
  a {
    transition: color cubic-bezier(0.075, 0.82, 0.165, 1) 1000ms;
    
    &:hover {
      color: rgb(83, 3, 3);

      span {
        color: white;
      }

      i {
        transition: transform cubic-bezier(0.075, 0.82, 0.165, 1) 500ms;
        transform: scale(1.5) ;
      }
    }
    .darkMode &:hover {
      color: black;
    }
  }
}

.social-links__github {
  svg {
    transition: transform cubic-bezier(0.075, 0.82, 0.165, 1) 1s;
  }
  span {
    transition: transform cubic-bezier(0.075, 0.82, 0.165, 1) 1s;
  }
  &:hover {
    svg {
      transform: translateX(100%);

    }
    span {
      transform: translateX(-100%);
    }

    @media (min-width: 640px) {
      svg {
        transform: translateX(80px);
      }
      span {
        transform: translateX(-30px);
      }
    }
  }

  @media (min-width: 640px) {
    padding-top: 0.8rem;
    padding-bottom: 0.5rem;
    padding-left: 1rem;
    padding-right: 1rem;
  }

  @media (min-width: 768px) {
    padding: 0.5rem;
  }
}
@color-start-1: #842f2f42;
@color-end-1: #ffcd7c;
@linear-gradient-1: linear-gradient(690deg, @color-start-1, @color-end-1);

.social-links-top {
  background: @linear-gradient-1;
}
</style>